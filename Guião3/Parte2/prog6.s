.data
 ...
 if:
 	beq $t5,1,else		# se bit sig for =1 ou seja maior que 8 salta se for =0 (menor que 8) faz
 	srl $s1,$s1,1		# deslocamento 
 	ori $s1,$s1,0x0001	# introduçao do bit mais significativo (8=1000)
 	
 	j endif
 else:
 	srl $s1,$s1,1		#deslocamento, nao se precisa de meter a introduçao do bit pois é unsigned este deslocamento
 endif:	
 	j while
 endwhile:
 	li $v0,0
